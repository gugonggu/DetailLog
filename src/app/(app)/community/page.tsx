  ...
  return (
    <main className="page-shell">
      <PageHeader
        eyebrow="Community"
        title="공개 세차 기록"
        description="공개로 설정된 세차 기록만 모아봅니다. 비공개 기록은 커뮤니티에 표시되지 않습니다."
        action={
      ...
      {pageError ? (
        <ErrorState className="mt-8" title="커뮤니티 기록을 불러오지 못했습니다." description={pageError.message} />
      ) : null}
      ...
      {!pageError && feedItems.length === 0 ? (
        <EmptyState
          ...
          title={filters.hasActiveFilters ? "조건에 맞는 공개 세차 기록이 없습니다." : "아직 공개된 세차 기록이 없습니다."}
          description={filters.hasActiveFilters ? "필터 조건을 바꾸거나 초기화해서 다시 확인해 보세요." : "세차 기록을 공개로 저장하면 이곳에서 다른 사용자가 함께 볼 수 있습니다."}
          action={filters.hasActiveFilters ? <Link className="secondary-action" href="/community">필터 초기화</Link> : undefined}
        />
      ) : null}
