...
type DeleteWashLogButtonProps = {
  washLogId: string;
  userId: string;
};

export function DeleteWashLogButton({ washLogId, userId }: DeleteWashLogButtonProps) {
  const router = useRouter();
  const [errorMessage, setErrorMessage] = useState("");
  const [isDeleting, setIsDeleting] = useState(false);

  async function handleDelete() {
    const confirmed = window.confirm("이 세차 기록을 삭제할까요?");
